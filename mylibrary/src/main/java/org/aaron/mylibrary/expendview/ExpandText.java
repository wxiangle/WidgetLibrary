...
public class ExpandText extends androidx.appcompat.widget.AppCompatTextView {
    ...
    //打开文字后面的图标
    private Bitmap openTextDrawable;
    //关闭文字后面的图标
    private Bitmap closeTextDrawable;
    ...
    public void setOnExpandCallback(ExpandText.OnExpandCallback callback) {
    ...
    private void init(Context context, AttributeSet attr) {
        ...
        if (attr != null) {
            ...
            setCloseTextDrawable(((BitmapDrawable) array.getDrawable(R.styleable.ExpandText_close_end_drawable)).getBitmap());
            setOpenTextDrawable(((BitmapDrawable) array.getDrawable(R.styleable.ExpandText_open_end_drawable)).getBitmap());
     ...
     * @param bitmap
     */
    public void setCloseTextDrawable(Bitmap bitmap) {
        this.closeTextDrawable = bitmap;
    }
     ...
     * @param bitmap
     */
    public void setOpenTextDrawable(Bitmap bitmap) {
        this.openTextDrawable = bitmap;
    }
        ...
        if (!isOpen) {
            ...
            if (closeHeight == 0) {
                ...
                SpannableStringBuilder sb = new SpannableStringBuilder(mCloseText)
                        .append(ellipseText)
                        .append(getEndSpannable()).append(getEndDrawableSpannable());
                ...
                while (closeLayout.getLineCount() > limitLine) {
                    ...
                    mCloseText = content.toString().substring(0, endIndex);
                    sb.clear();
                    sb.append(mCloseText).append(ellipseText).append(getEndSpannable()).append(getEndDrawableSpannable());
            ...
            //展开状态
            //有过记录，不用重新测量
            SpannableStringBuilder sb = new SpannableStringBuilder(content).append(getEndSpannable()).append(getEndDrawableSpannable());
            if (openHeight == 0) {
    ...
    private SpannableString getEndDrawableSpannable() {
        SpannableString end = new SpannableString(isOpen ? "上" : "下");
        Bitmap bitmap = null;
        try {
            if (isOpen) {
                bitmap = openTextDrawable;
            } else {
                bitmap = closeTextDrawable;
            }
            OffsetImageSpan imageSpan = new OffsetImageSpan(getContext(), LxDensityUtil.INSTANCE.dip2px(getContext(), 3f), bitmap, DynamicDrawableSpan.ALIGN_CENTER);
            ...
            end.setSpan(imageSpan, 0, end.length(), SPAN_INCLUSIVE_EXCLUSIVE);
        } catch (Exception e) {
            Log.d("e ", "");
            end = new SpannableString("");
        }
        return end;
    }
